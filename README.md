...
6. For each cluster, find methods that have successfully located it. Plot 2D coordinates (obtained earlier with TSNE) of each conformer and color them according to status of the cluster (found only by MCR/only alternative/both).
...
| `create_testset.py` | Assemble testset info for MCR, MTD and RDKit (all 150 molecules) |
...
| *Auxiliary files* |  |
| `compute_niso.py` | Generate isomorphism counts to be used by MCR for efficient RMSD filtering |
