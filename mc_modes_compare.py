...
SAMPLING_MODES = {
    'ringoMC': '-rmsd -eweak',
    'ringoMCR': '-rmsd -eweak -mcrsmart -vara 5 -varb 1',
}
...
def main(env_name):
    ...
    DF_FILENAME = f'{RINGO_METHOD}_mcmode.csv'

    import sys, json
    import pandas as pd
    import ringo
    from chemscripts.geom import Molecule
    assert ringo.build_flags == SAMPLING_MODES[env_name]
...
if __name__ == "__main__":
    ...
    assemble_dir = os.path.abspath('../release_assemble')
    for mode_name, build_flags in SAMPLING_MODES.items():
        env.build_ringo('intel', build_flags, assemble_dir)
        env.exec(__file__, func=main, env='intel', args=(mode_name,))
