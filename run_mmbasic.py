...
OUTPUT_DIR = f'{MMBASIC_METHOD}_conformers'
CPULOAD_JSON = f'cpuload_{MMBASIC_METHOD}.json'
CPULOAD_DATA = {}
INPUTFILE_TEMPLATE_FNAME = './mmgeneral_setup'
INPUTFILE_TEMPLATE = open(INPUTFILE_TEMPLATE_FNAME, 'r').read()
...
def convert_structures(start_name, result_name):
    if start_name.endswith('sdf') and result_name.endswith('mae'):
        startext, resext = 'sdf', 'mae'
    elif start_name.endswith('mae') and result_name.endswith('sdf'):
...
def gen_mmbasic(molname, sdf_name, p):
    import psutil
    import numpy as np
    ...
    sdflines = open(start_sdffile, 'r').readlines()
    
    # MacroModel is better at figuring charges out when SDF does not contain them
    for i, line in enumerate(sdflines):
        if "CHG" in line:
            del sdflines[i]
    with open(start_sdffile, 'w') as f:
        f.write(''.join(sdflines))
    ...
    result_short = f'{molname}_result.mae'
    input_data = INPUTFILE_TEMPLATE.format(input=start_short, output=result_short)
    input_fname = os.path.join(calc_dir, 'input')
    with open(input_fname, 'w') as f:
        f.write(input_data)

    mm_process = subprocess.Popen(f"./exec_mmbasic {calc_dir}", shell = True)
    ...
    # Read SDF into Confpool through OpenBabel intermediate
...
def main():
    ...
    for molname, sdf_name in input_data.items():
        ...
        with TimingContext(timer, molname):
            ntotal = gen_mmbasic(molname, sdf_name, p)
        ...
        df['method'].append(MMBASIC_METHOD)
        df['testcase'].append(molname)
        df['nconf'].append(ntotal)
        df['nunique'].append(len(p))
        df['time'].append(timer.time_elapsed)

        result_xyz = os.path.join(OUTPUT_DIR, f"{molname}_{MMBASIC_METHOD}.xyz")
        p.save(result_xyz)
        timer.finish_iter()
        # except Exception as e:
        #     print(f"Failed on {molname}", flush=True)
        #     traceback.print_exc()
        #     timer.finish_iter()
